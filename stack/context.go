...
func (s *scanningState) scan(line []byte, opts *Opts) (bool, error) {
	...
	case gotFunc:
		...
		if found, err := parseFile(&cur.Stack.Calls[len(cur.Stack.Calls)-1], trimmed, opts); err != nil {
			return false, err
		} else if !found {
	...
	case gotCreated:
		if found, err := parseFile(&cur.CreatedBy.Calls[0], trimmed, opts); err != nil {
			return false, err
		} else if !found {
	...
	case gotFileFunc:
		if match := reCreated.FindSubmatch(trimmed); match != nil {
			...
			cur.CreatedBy.Calls[0].init("", 0, 0)
			s.state = gotCreated
			return true, nil
		}
	...
	case gotRaceGoroutineFunc:
		...
		if found, err := parseFile(&c[len(c)-1], trimmed, opts); err != nil {
			return false, err
		} else if !found {
			return false, fmt.Errorf("expected a file after a race function, got: %q", trimmed)
		}
...
//
// Uses reFile.
func parseFile(c *Call, line []byte, opts *Opts) (bool, error) {
	if match := reFile.FindSubmatch(line); match != nil {
		...
		var pcOffset int64
		if opts.ParsePC {
			pc, err := strconv.ParseInt(string(match[3]), 0, 64)
			if err != nil {
				return true, fmt.Errorf("failed to parse pc on line: %q: %w", bytes.TrimSpace(line), err)
			}
			pcOffset = pc
		}
		c.init(string(match[1]), num, pcOffset)
		return true, nil
	}
	return false, nil
}
